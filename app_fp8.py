...
config_file_path = "configs/ltxv-13b-0.9.7-distilled-fp8.yaml"
...
def encode_prompt(prompt, negative_prompt):
    """
    Encodes the prompt and negative prompt into a format suitable for the pipeline.
    This is a placeholder function; actual encoding logic will depend on the pipeline's requirements.
    """
    # --- Precompute text embeddings and attention masks ---
    # Use pipeline_instance.tokenizer and pipeline_instance.text_encoder
    with torch.no_grad():
...
def generate(prompt, negative_prompt, input_image_filepath, input_video_filepath,
    ...
    (
        ...
        negative_prompt_embeds,
        negative_attention_mask
    ) = encode_prompt(prompt, negative_prompt)
...
if __name__ == "__main__":
    ...
    demo.queue().launch(debug=True, share=False, mcp_server=False, server_name="0.0.0.0", server_port=7860)